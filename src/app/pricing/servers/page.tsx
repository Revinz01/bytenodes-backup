...
const premiumPackages = [
  {
    ...
    database: 2,
    port: 2,
    note: "Dedicated/Pinned Threads (100% - 200% Guarantee)",
  },
  {
    name: "Premium-2",
    price: 30000,
...
export default function PricingServerPage() {
  ...
  const getCategoryTitle = () => {
    switch (selectedCategory) {
      ...
      case "premium":
        return {
          title: "Premium Packages (SG - Performance)",
          desc: "Anti-Lag, Prioritas Tinggi, Resource Lega - Rp 15.000/GB",
        };
      case "dedicated-eu":
        return {
            ...
            >
              {getPackages().map((pkg, index) => (
                <motion.div key={index} variants={scrollVariants}>
                  <Card
                    className={`p-6 relative hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 ${
                      pkg.popular
                        ? "border-primary shadow-lg border-2 bg-gradient-to-b from-card to-primary/5"
